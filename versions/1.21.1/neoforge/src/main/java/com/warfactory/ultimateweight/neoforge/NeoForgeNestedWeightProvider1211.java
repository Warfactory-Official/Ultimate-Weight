...
/**
 * NeoForge nested-container weight. Reads, in order: the standard item-handler capability (live
 * stacks - covers backpacks and most mod containers), the modern {@code minecraft:container}
 * component (vanilla shulker boxes), then the legacy {@code BlockEntityTag.Items} NBT list. The
 * latter two are surfaced through {@link ItemNbtBridge1211}; only the legacy NBT path needs registry
 * access for deserialization.
 */
@CompatPlugin
@SuppressWarnings("unused")
public final class NeoForgeNestedWeightProvider1211 implements IWeightCompatProvider {
