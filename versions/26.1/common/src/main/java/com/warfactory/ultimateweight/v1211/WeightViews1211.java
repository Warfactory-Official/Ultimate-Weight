...
public final class WeightViews1211 {
     ...
     * their weight cannot be cached by the item component hash and a single-slot delta cannot be
     * trusted. Set once by the loader layer; defaults to "never".
     */
    private static volatile Predicate<ItemStack> dynamicContainer = stack -> false;

    private WeightViews1211() {
    }
    ...
    private static final class StackView implements WeightStackView {
        ...
        public String complexCacheKey() {
            // Backpack contents live in a capability / world SavedData, not in the item components, so
            // the component-hash cache key would never change when the contents do and the cached
            // weight would freeze. Returning null bypasses the cache so live contents are re-read.
    ...
    private record DataView(ItemStack stack) implements WeightDataView {
        ...
        @Override
            public Double getDouble(String key) {
                CompoundTag tag = ItemNbtBridge1211.customData(stack);
    ...
    private static String complexKey(ItemStack stack, int depth) {
        return BuiltInRegistries.ITEM.getKey(stack.getItem()) + "|" + depth + "|" + ItemNbtBridge1211.signature(stack);
    }
}
