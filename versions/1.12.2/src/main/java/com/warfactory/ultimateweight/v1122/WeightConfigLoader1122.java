...
public final class WeightConfigLoader1122 implements IConfigLoader {
    ...
    private static EquipmentBonusRules equipmentBonuses(Map<String, Object> bonuses, EquipmentBonusRules fallback) {
        ...
        for (Map.Entry<String, Object> entry : exact.entrySet()) {
            ...
            if (!bonus.isEmpty()) {
                putLegacyBonus(builder, entry.getKey(), bonus);
            }
        }
    ...
    private static void putLegacyExact(InventoryGroupRules.Builder builder, String groupId, String key) {
        if (key == null) {
            return;
        }
        String trimmed = key.trim();
        int separator = trimmed.lastIndexOf('@');
        if (separator < 0) {
            builder.addExact(groupId, trimmed, 0);
            return;
        }
        String itemId = trimmed.substring(0, separator).trim();
        String metaToken = trimmed.substring(separator + 1).trim();
        if ("*".equals(metaToken)) {
            builder.addWildcard(groupId, itemId);
            return;
        }
        try {
            builder.addExact(groupId, itemId, Integer.parseInt(metaToken));
        } catch (NumberFormatException ignored) {
        }
    }

    private static void putLegacyBonus(EquipmentBonusRules.Builder builder, String key, EquipmentBonusRules.EquipmentBonus bonus) {
        if (key == null) {
            return;
        }
        String trimmed = key.trim();
        int separator = trimmed.lastIndexOf('@');
        if (separator < 0) {
            builder.putExact(trimmed, 0, bonus);
            return;
        }
        String itemId = trimmed.substring(0, separator).trim();
        String metaToken = trimmed.substring(separator + 1).trim();
        if ("*".equals(metaToken)) {
            builder.putWildcard(itemId, bonus);
            return;
        }
        try {
            builder.putExact(itemId, Integer.parseInt(metaToken), bonus);
        } catch (NumberFormatException ignored) {
        }
    }
    ...
    private static void copyGroups(InventoryGroupRules.Builder builder, InventoryGroupRules rules) {
        ...
        for (Map.Entry<String, List<String>> entry : rules.exactMatches().entrySet()) {
            for (String groupId : entry.getValue()) {
                putLegacyExact(builder, groupId, entry.getKey());
            }
        }
    ...
    private static void copyEquipmentBonuses(EquipmentBonusRules.Builder builder, EquipmentBonusRules rules) {
        for (Map.Entry<String, EquipmentBonusRules.EquipmentBonus> entry : rules.exactBonuses().entrySet()) {
            putLegacyBonus(builder, entry.getKey(), entry.getValue());
        }
    ...
    private static Map<String, Object> groupLimitsMap(WeightConfig config) {
        ...
        for (InventoryGroupRules.GroupDefinition definition : rules.definitions()) {
            ...
            group.put("exact", entriesForGroup(rules.exactMatches(), definition.id()));
            group.put("wildcards", entriesForGroup(rules.wildcardMatches(), definition.id()));
            group.put("oredict", entriesForGroup(rules.dictionaryMatches(), definition.id()));
    ...
    private static Map<String, Object> equipmentBonusesMap(WeightConfig config) {
        ...
        for (Map.Entry<String, EquipmentBonusRules.EquipmentBonus> entry : config.equipmentBonusRules().exactBonuses().entrySet()) {
            exact.put(entry.getKey(), equipmentBonusMap(entry.getValue()));
        }
    ...
    private static List<String> entriesForGroup(Map<String, List<String>> entries, String groupId) {
